...
bool ONNXRuntimePredictor::InitBinding() {
  ...
  int device_id = 0;
  if (config_.use_gpu()) {
    device_id = config_.gpu_device_id();
    place_ = paddle::platform::CUDAPlace(device_id);
  } else {
  ...
  Ort::MemoryInfo memory_info(device_name, OrtDeviceAllocator, device_id,
                              OrtMemTypeDefault);
  ...
  for (size_t i = 0; i < n_outputs; ++i) {
    ...
    Ort::MemoryInfo out_memory_info(device_name, OrtDeviceAllocator, device_id,
                                    OrtMemTypeDefault);
...
Ort::Value ONNXRuntimePredictor::GetOrtValue(const ONNXDesc &desc,
  ...
  std::vector<int64_t> shape =
      paddle::framework::vectorize<int64_t>(tensor->dims());
  return Ort::Value::CreateTensor(memory_info, tensor->data<void>(), size,
